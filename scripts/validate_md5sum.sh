...
for i in ./data/illumina_data_reads/*.gz; do
  ...
  if echo "$i" | grep -q "_R1_"; then
    ...
    expected="$(cat ./data/illumina_data_reads/DNA_sequencing_stats.csv | grep "^$sample" | cut -d , -f6 | tr -d '\r[:space:]"')"
  else
    # expected R2 md5 is column 7
    expected="$(cat ./data/illumina_data_reads/DNA_sequencing_stats.csv | grep "^$sample" | cut -d , -f7 | tr -d '\r[:space:]"')"
  fi

  # actual md5 from the file
  actual="$(md5sum "$i" | cut -d ' ' -f1 | tr -d '\r[:space:]')"
